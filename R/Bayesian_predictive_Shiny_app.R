#' @name Bayesian_predictive_Shinny_app
...
Bayesian_predictive_Shinny_app <- function() {
  appDir <- system.file("shiny-examples", "myapp", package = "BayesianPredictiveFutility")
  if (appDir == "") {
    stop("Could not find example directory. Try re-installing `BayesianPredictiveFutility`.", call. = FALSE)
  }

  shiny::runApp(paste(appDir,'/Bayesian_Predictive_futility_Shiny.R',sep=''), launch.browser =T,host = getOption( "127.0.0.1"))
}
