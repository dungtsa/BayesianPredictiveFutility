...
# Table 2: Bayesian Predictive Probability 
...
```{r, echo = FALSE, results = 'asis'}
    ...
    for (my_j in 1:length(aa.list))
    {
      ...
      dimnames(aa)[[2]] <- c(paste('number of patients with ', outcomeLabel,' in the ',stage.index,' interim analysis',sep = ''),paste('minimum number of patients with ',outcomeLabel,' needed in the future remaining patients',sep = ''),'predictive probability')
      print(knitr::kable(aa))
  ...
  tmp3.list <- res$predictive.prob.data
    for (my_j in 1:length(tmp3.list)) {
      ...
      stage.index <- ifelse(my_j == 1,' 1st ',ifelse(my_j == 2,' 2nd ',ifelse(my_j == 3,' 3rd ',paste(' ',my_j,'th ',sep = ''))))
        ...
        mtext(paste(ifelse(increase, 'minimum', 'maximum'),' number of patients with ',outcomeLabel,' ',ifelse(increase, 'needed', 'allowed'),' in the future remaining ', n2,' patients',sep = ''),side = 3,line = 1.8)
...
`r if (run_fig_3) '# Figure 3: Probability of Stopping Early by Each Interim Analysis'`

```{r, echo = FALSE, results = 'asis',fig.height = 16,fig.width = 14,dpi = 600, eval = ifelse(run_fig_3,T,F)}
    res  <-   tmp99
    tmp5 <- res$sensitivity.data
    tmp5.ind <- res$sensitivity.data.ind
    n.list <- res$n.list
    ...
    dim.n <- dim(tmp5.ind)[2]
      ...
      for (my_j in 1:(dim.n - ifelse(increase,2, 1)))
      {
