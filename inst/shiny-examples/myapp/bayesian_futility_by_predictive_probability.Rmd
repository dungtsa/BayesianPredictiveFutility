  ...
  tmp3.list <- res$predictive.prob.data
    for (my_j in 1:length(tmp3.list)) {
      ...
      stage.index <- ifelse(my_j == 1,' 1st ',ifelse(my_j == 2,' 2nd ',ifelse(my_j == 3,' 3rd ',paste(' ',my_j,'th ',sep = ''))))
        ...
        mtext(paste(ifelse(increase, 'minimum', 'maximum'),' number of patients with ',outcomeLabel,' ',ifelse(increase, 'needed', 'allowed'),' in the furture remaining ', n2,' patients',sep = ''),side = 3,line = 1.8)
